...
class TallerModel extends Model
{
    ...
    {
        $builder = $this->db->table('cita C');
        $builder->select("COUNT(C.idCita) AS 'total', (SELECT COUNT(CI.idCita) FROM cita CI WHERE CI.estado = -1 AND CI.idTaller = C.idTaller AND CI.fechaCreacion BETWEEN '".$fechaInicio."' AND '".$fechaFinal."'".") AS 'rechazado', (SELECT COUNT(CI.idCita) FROM cita CI WHERE CI.estado = 0 AND CI.idTaller = C.idTaller AND CI.fechaCreacion BETWEEN '".$fechaInicio."' AND '".$fechaFinal."'".") AS 'pendiente',(SELECT COUNT(CI.idCita) FROM cita CI WHERE CI.estado = 1 AND CI.idTaller = C.idTaller AND CI.fechaCreacion BETWEEN '".$fechaInicio."' AND '".$fechaFinal."'".") AS 'atencion', (SELECT COUNT(CI.idCita) FROM cita CI WHERE CI.estado = 2 AND CI.idTaller = C.idTaller AND CI.fechaCreacion BETWEEN '".$fechaInicio."' AND '".$fechaFinal."'".") AS 'finalizado', (SELECT COUNT(CI.idCita) FROM cita CI WHERE CI.estado = 3 AND CI.idTaller = C.idTaller AND CI.fechaCreacion BETWEEN '".$fechaInicio."' AND '".$fechaFinal."'".") AS 'inasistencia'");
        $where = " C.idTaller ='".$idTaller."' AND C.fechaCreacion BETWEEN '".$fechaInicio."' AND '".$fechaFinal."'";
        $builder->where($where);
        $query = $builder->get();
        return $query->getResult();
